...
public class RequestServiceImpl2 {
	public static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(RequestServiceImpl2.class);
	...
	public static List<D009040> getMinistatement(int lbrCode, String prdAcctId, int noofstmts) {
		...
		criteria.add(Restrictions.le("postDate", DateUtil.getFormattedDateNew(new Date())));
	...
	public static BalanceResponse fetchBalance(String brCode, String accNo) {
		...
		if (list != null && list.size() > 0) {
			...
			if (list.get(0).getFcyScheme().trim().equalsIgnoreCase(MSGConstants.FCY_SCHEME)) {
				balanceResponse.setAccActualBalance(String.valueOf(
						new BigDecimal(Double.valueOf(lists.get(0)) + Double.valueOf(list.get(0).getFlexiBalance())
								- Double.valueOf(list.get(0).getTotalLienFcy())).setScale(2, BigDecimal.ROUND_HALF_UP)));
	...
	public static CustomerDetails getCustomerDetails(String mobno) {
		...
		try {
			...
			String miniStmt = "";
			List<D009040> stmt = RequestServiceImpl2.getMinistatement(
			...
			if (stmt != null && !stmt.isEmpty()) {
				for (int i = 0; i < stmt.size(); i++) {
					miniStmt +=  (DateUtil.convertDateFormatIVR(stmt.get(i).getPostDate())) + ": " + stmt.get(i).getFcyTrnAmt() + " "
							+ stmt.get(i).getDrCr() + "," + stmt.get(i).getParticulars().trim() + "\n";//
				}
				customerDetails.setResponse(MSGConstants.SUCCESS);
	...
	public static OtherChannelServiceResponse storePinOffset(String custNo, String pinType, String offset, String pin,
		...
		if (channel.equalsIgnoreCase(MSGConstants.IMPS_CHANNEL)) {
			...
			if (list == null || list.size() < 1) {
				 ...
				 * ClientConfig().register( LoggingFilter.class ) ); WebTarget
				 * webTarget =
				 * client.target("http://10.100.5.238:9090/AccountOpening/").
				 * path("request").path("fetchAccounts").queryParam("param1",
				 * custNo.trim());
				 * 
			...
			try {
				...
				response.setErrorMessage("Dear Customer, your have successfully generated mmid as " + mmid
						...
						+ Integer.valueOf(sourceAccount.getId().getPrdAcctId().substring(
								sourceAccount.getId().getPrdAcctId().length()
										- 16,
	...
	public static OtherChannelServiceResponse getChequeStatus(String custNo, String mmidNo, String chequeNo,
		...
		/*
		 * if(issuedInstruments.getStatus() == Byte.valueOf("2")){ D09040
		 * voucher =
		 ...
		 * issuedInstruments.getSetNo(), issuedInstruments.getScrollNo());
		 * if(null != voucher){ particular = voucher.getParticulars(); } }
		 */
	...
	public static IMPSFetchLoanAccountDetailsResponse loanAccountDetails(int lbrCode, String accNo) {
		...
		for (Iterator<Object[]> iterator = loanList.iterator(); iterator.hasNext();) {
			...
			BigDecimal three = BigDecimal.valueOf((Double) object[2]);
			BigDecimal four = BigDecimal.valueOf((Double) object[3]);
			Date five = (Date) object[4];
			BigDecimal six = BigDecimal.valueOf(Long.valueOf(object[5] + ""));
			BigDecimal seven = BigDecimal.valueOf((Double) object[6]);
	...
	public static IMPSFetchDepositeReceiptResponse fetchDepositAccReceipt(int lbrCode, String accNo) {
		...
		for (D020004 termDepositReceipts : receiptList) {
			...
			depositeRec.setReceiptNo(termDepositReceipts.getId().getPrdAcctId());
	...
	public static OtherChannelServiceResponse impsOTPGeneration(String custNo, String mmid, String transAmount,
		...
		if (channel.trim().equalsIgnoreCase(MSGConstants.IMPS_CHANNEL)) {
			try {
				...
				/*
				 * String smsMsg =
				 * "Dear Customer, Your Dynamic PIN for Transaction is "
				 * +otp+". The Dynamic PIN is valid for 5 mins."; DeliverySMS
				 * sms = wsValidation.prepareDeliverySMSObject(account.getId().
				 * getLbrcode(), custxTreDtls.getId().getMobileno(),
				 * DateUtility.convertDateFormat(new Date()),
				 * DateUtility.getFormattedTime(new Date()),
				 * "OtpGen_"+DateUtility.getcurrentDateForPDF(), custNo, smsMsg,
	...
	public static PigmeAccountsResponse searchPigMeAccCust(String accNo) {
		...
		try {
			...
			ArrayList<String> acList = new ArrayList<String>();
			for (D047003 d047003 : list) {
				System.out.println("d047003.getId().getCustPrdAcctId():::>>" + d047003.getId().getCustPrdAcctId());
				acList.add(d047003.getId().getCustPrdAcctId());
	...
	public static DepositeAcctOpenResponse openTermDepositAccount(DepositeAcctOpenRequest request) {
		DepositeAcctOpenResponse response = new DepositeAcctOpenResponse();
		...
		List<D009022> list = criteria.list();
		System.out.println("List of Accounts ::>>>" + list);
		...
		if (request.getAccType().trim().equalsIgnoreCase("CR")) {
			...
			System.out.println("Response of validate account::>>>" + res);
			logger.error("Response of validate account::>>>" + res);
			if (res != null && res.getResponse().equalsIgnoreCase(MSGConstants.ERROR)) {
				response.setResponse(MSGConstants.ERROR);
				response.setErrorMessage(res.getErrorMsg());
				return response;
			}else request.setDebitAccount(sourceAccount.getId().getPrdAcctId());
			// =========================================
			boolean isValid = false;
			logger.error("Same Branch Fund Transfer....");
			System.out.println("Same Branch Fund Transfer....");
			int setNo = VoucherCommon.getNextSetNo();
			int mainScrollNo = VoucherCommon.getNextScrollNo();
			String rrn = DataUtils.getNextRRN();
			common.debitSameBranch(Integer.parseInt(request.getBrcode()), request.getDebitAccount(), MSGConstants.IMPS_CHANNEL,
					setNo, VoucherCommon.getNextScrollNo(), "IMPS/TD/" + request.getProdCode(), Double.parseDouble(request.getDepositAmount()),
					rrn, mainScrollNo, session);
			// common.balance(request.getLbrCode().intValue(),
			// request.getAccountNo(), request.getAmount(), "D");
			CoreTransactionMPOS.balance(Integer.parseInt(request.getBrcode()), sourceAccount.getId().getPrdAcctId().trim(),
			...
			// =========================================
			if (isValid) {
				String accNo = CustomerMasterServiceImpl.getNextTDAccNo(request.getProdCode().trim(),
				...
				d009022.setAcctStat(Byte.valueOf("2"));
				d009022.setAcctType(Byte.valueOf("" + Integer.valueOf("2")));
				...
				criteria3.add(Restrictions.eq("days", Short.valueOf(request.getDays().toString())));
				criteria3.add(Restrictions.eq("months", Short.valueOf(request.getMonth().toString())));
				criteria3.add(Restrictions.eq("id.prdCd", request.getProdCode().trim()));
				criteria3.addOrder(Order.desc("id.intEffDt"));
				List<D020118> list2 = criteria3.list();
				if (list2 == null || list2.isEmpty()) {
					response.setResponse(MSGConstants.ERROR);
					response.setErrorMessage(MSGConstants.INT_RATE_NOT_FOUND);
					return response;
				}
				...
				int lbrcode = Integer.parseInt(request.getBrcode());
				...
				double matValue = DataUtils.getMatValue(Double.parseDouble(request.getDepositAmount()), rateOfint, actNoOfDays);
				...
				d020004.setReceiptStatus(Byte.valueOf("1"));
				...
				d020004.setMainBalLcy(0);
				d020004.setMainBalFcy(0);
				...
				try {
					common.creditSameBranch(d009022.getId().getLbrCode(),d009022.getId().getPrdAcctId(), MSGConstants.IMPS_CHANNEL, setNo ,
							VoucherCommon.getNextScrollNo(),"IMPS/TD/"+request.getProdCode(), Double.parseDouble(request.getDepositAmount()),rrn,mainScrollNo,session);
					session.save(d009022);
					session.save(d020004);
					
					 //CoreTransactionMPOS.balance(d009022.getId().getLbrCode(),d009022.getId().getPrdAcctId(),request.getAmount(), "C");
					...
					response.setResponse(MSGConstants.SUCCESS);
					response.setErrorMessage(MSGConstants.SUCCESS_MSG);
					response.setCustNo(accNo);
					//response.setTransactionId(rrn);
					//response.setMobileNo(d020004.getId().getPrdAcctId());
			...
			int mainScrollNo = VoucherCommon.getNextScrollNo();
			String rrn = DataUtils.getNextRRN();
			common.debitSameBranch(Integer.parseInt(request.getBrcode()), sourceAccount.getId().getPrdAcctId(), MSGConstants.IMPS_CHANNEL,
					setNo, VoucherCommon.getNextScrollNo(), "IMPS/TD/" + request.getProdCode(), Double.parseDouble(request.getDepositAmount()),
					rrn, mainScrollNo, session);
			...
			if (!common.isAborted) {
				logger.error("Transaction successful");
				common.creditSameBranch(Integer.parseInt(request.getBrcode()), MSGConstants.ABB_ACC,
						MSGConstants.IMPS_CHANNEL, setNo, VoucherCommon.getNextScrollNo(),
						"IMPS/TD/" + request.getProdCode(), Double.parseDouble(request.getDepositAmount()), rrn, mainScrollNo, session);
				// CoreTransactionMPOS.balance(request.getLbrCode().intValue(),MSGConstants.ABB_ACC,
				// request.getAmount(), "C");//
				response.setResponse(MSGConstants.SUCCESS);
				response.setErrorMessage(MSGConstants.SUCCESS_MSG);
			...
			// =========================================
			if (isValid) {
				System.out.println("request.getProductCode().substring(0,8).trim()::>>>>"
						+ request.getProdCode().trim());
				logger.error("request.getProductCode().trim()::>>>>"
						+ request.getProdCode().trim());
				Criteria criteria3 = session.createCriteria(D020118.class);
				criteria3.add(Restrictions.eq("days", Short.valueOf(request.getDays().toString())));
				criteria3.add(Restrictions.eq("months", Short.valueOf(request.getMonth().toString())));
				criteria3.add(Restrictions.eq("id.prdCd", request.getProdCode()));
				criteria3.addOrder(Order.desc("id.intEffDt"));
				List<D020118> list2 = criteria3.list();
				if (list2 == null || list2.isEmpty()) {
					session.close();
				...
				Query q = session
						.createSQLQuery("SELECT max(PrdAcctId) FROM D020004 WHERE LBrCode='"+Integer.parseInt(request.getBrcode())+"' AND PrdAcctId like '"
								+ sourceAccount.getId().getPrdAcctId().trim().substring(0, 24) + "%" + "'");
				System.out.println("q.getResultList().get(0)::>>>" + q.getResultList().get(0));
				System.out.println("" + Long.valueOf(String.valueOf(q.getResultList().get(0)).substring(16, 32)) + 1l);
				Long recieptNo = Long.valueOf(String.valueOf(q.getResultList().get(0)).substring(16, 32)) + 1l;
				System.out.println("recieptNo::>>" + recieptNo);
				logger.error("recieptNo::>>>" + recieptNo);
				String recNo = sourceAccount.getId().getPrdAcctId().substring(0, 16) + String.format("%016d", recieptNo);
				...
				D020004Id id = new D020004Id();
				id.setLbrCode(lbrcode);
				id.setPrdAcctId(recNo);
				double matValue = DataUtils.getMatValue(Double.parseDouble(request.getDepositAmount()), rateOfint, actNoOfDays);
				logger.error("Maturity value::>>" + matValue);
				System.out.println("Maturity value::>>" + matValue);
				d020004.setAccPrvdAmtFcy(0);
				d020004.setAccPrvdAmtLcy(0);
				d020004.setAsOffdate(DateUtil.getCurrentDate());
				d020004.setBrokenPeriodInt(0);
	...
	public static CustNewMobileDetails newEmailId(String emailId, String custNo){
		...
		if(d350078==null) {
			...
			d350078 = new D350078(id, "IN", " ", " ", emailId, zero, zero, zero, zero, zero, zero, new Date(), zero);
		...
		Query<D350078> query1 = session.createQuery(
				"UPDATE D009011 SET EmailId = '" + emailId.trim() + "' WHERE CustNo = '" + d009011.getCustno() + "'");
		j= query1.executeUpdate();
		t.commit();
		session.close();
		//session = null;
	...
	/*public static List<Object[]> getFDProductList(String custno) {
		...
		String queryString ="select A.id.prdCd ,A.name, B.minDepAmt, B.maxDepAmt, B.minPeriod, B.maxPeriod, B.periodType FROM D009021 A, D020002 B WHERE A.id.lbrCode=B.id.lbrCode AND A.id.prdCd=B.id.prdCd and A.moduleType=:moduleType AND A.acctStat=:accstat  AND A.id.lbrCode=:brcode" ;
				short mtype=20;
				int lbrcode=(int) brCode;
				Byte accstat=1;
		Query q = session.createQuery(queryString).setParameter("moduleType", mtype).setParameter("brcode", lbrcode).setParameter("accstat", accstat);
